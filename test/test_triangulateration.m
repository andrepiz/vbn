...
ranges = extract_ranges_bodies_kernel(rr_observer, time_UTC, id_beacons, filename_metakernel, ltscorrection, origin, frame);

%% MEASUREMENTS
rng(0)
sigma_ranges = 0.02; % [%] from RION (planets)
sigma_ranges = 0.002; % [%] from RION (planets)
...
ranges_pert = ranges.*(1 + sigma_ranges*randn(size(ranges)));
...
rr_solution_nl = nan(3, nt);
for ix = 1:nt
    for ib = 1:length(n_beacons)
        rr_solution_mean = triangulateration(squeeze(loses_pert(:, ix, 1:n_beacons(ib))), squeeze(ranges_pert(:, ix, 1:n_beacons(ib)))', squeeze(rr_beacons_pert(:, ix, 1:n_beacons(ib))));
        rr_solution(:, ix, ib) = rr_solution_mean;
        %rr_solution_nl(:, ix, ib) = nonlineartriangulateration(squeeze(loses_pert(:, ix, 1:n_beacons(ib))), squeeze(ranges_pert(:, ix, 1:n_beacons(ib)))', squeeze(rr_beacons_pert(:, ix, 1:n_beacons(ib))));
    end
    disp([num2str(1e2*ix/nt),'%'])
end
...
cm2 = interp1(linspace(0, 1, size(cm2, 1)), cm2, linspace(0, 1, n_beacons(end)));

figure('name','measurement'), grid on, hold on, 
for ib = 1:n_beacons(end)
    plot(time_UTC, ranges_pert(:,:,ib)*k,'Color',cm2(ib,:), 'DisplayName',[num2str(ib)])
    plot(time_UTC, ranges(:,:,ib)*k,'r--','DisplayName','True'), 
end
legend show
xlabel('date')
ylabel('measured range [AU]')

figure('name','measurement_error'), grid on, hold on, 
for ib = 1:n_beacons(end)
    plot(time_UTC, abs(ranges_pert(:,:,ib) - ranges(:,:,ib))*k,'Color',cm2(ib,:), 'DisplayName',[num2str(ib)])
end
set(gca(), 'YScale','log')
legend show
xlabel('date')
ylabel('range error [AU]')
...
hold on, grid on
for ib = 1:length(n_beacons)
    scatter(n_beacons(ib), 1e-3*mean(vecnorm(rr_solution(:,:,ib) - rr_observer)),'o','filled')
end
set(gca(), 'YScale','log')
ylim([1e3 5e6])
xlim([0 11])
...
% figure('name','triangulation_error_nl','Units','normalized','Position',[0.1 0.1 0.3 0.4])
% hold on, grid on
% for ib = 1:length(n_beacons)
%     scatter(ib, 1e-3*mean(vecnorm(rr_solution_nl(:,:,ib) - rr_observer)),'o','filled','DisplayName',num2str(n_beacons(ib)))
% end
% set(gca(), 'YScale','log')
% legend show
% ylim([1e3 5e6])
...
% ylabel('triangulated position error [km]')
% 
% figure('name','solution_difference_nl_mean','Units','normalized','Position',[0.1 0.1 0.8 0.4])
% hold on, grid on
% for ib = 1:length(n_beacons)
%     plot(days(time_UTC - time_UTC(1)), 1e-3*vecnorm(rr_solution(:,:,ib) - rr_solution_nl(:,:,ib)),'DisplayName',[num2str(n_beacons(ib)),'B'])
% end
% set(gca(), 'YScale','log')
% legend show
% xlabel('days')
% ylabel('solution difference [km]')
