...
[ranges, rr_beacons] = extract_ranges_bodies_kernel(rr_observer, time_UTC, id_beacons, filename_metakernel, ltscorrection, origin, frame);

%% MEASUREMENTS
rng(0)
sigma_ranges = 0.02; % [%] from RION (planets)
sigma_ranges = 0.00001; % [%] to have performance similar with classical triangulation (planets)
...
for ix = 1:nt
    for ib = 1:length(n_beacons)
        ...
        %     nt
        % end
        rr_solutions = lateration(squeeze(ranges_pert(:, ix, 1:n_beacons(ib)))', squeeze(rr_beacons_pert(:, ix, 1:n_beacons(ib))));
        if ib == 1
            if ix == 1
                [~, ix_solution] = min(vecnorm(rr_solutions - rr_observer(:, ix)));
                rr_solution(:, ix, ib) = rr_solutions(:, ix_solution);
            else
                [~, ix_solution] = min(vecnorm(rr_solutions - rr_solution(:, ix-1, ib)));
                rr_solution(:, ix, ib) = rr_solutions(:, ix_solution);
            end
        else
...
for ii = 1: 4%length(name_beacons)
...
hold on, grid on
for ib = 1:length(n_beacons)
    scatter(n_beacons(ib), 1e-3*mean(vecnorm(rr_solution(:,:,ib) - rr_observer),'omitnan'),'o','filled')
end
set(gca(), 'YScale','log')
%legend show
% ylim([1e3 5e6])
